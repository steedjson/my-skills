...
- `[model_providers.codex_model_router_v2]` 中内嵌的 `models` 数组。
...
1. 先运行检查，不写入文件：
   ...
   ```bash
   python3 /path/to/cc-switch-reasoning-tier-repair/scripts/repair.py --check
   ```

2. 用户明确要求修复时，先预览：

   ```bash
   python3 /path/to/cc-switch-reasoning-tier-repair/scripts/repair.py --dry-run
   ```

3. 预览结果符合预期后，去掉 `--dry-run` 执行修复。脚本会在写入前备份 `config.toml` 和 catalog，并在写入后重新解析 TOML 与 JSON。

4. 使用 `--config` 指定配置文件，使用 `--backup-dir` 指定备份目录；需要其他已核实模型映射时，用 `--tier-map` 指定 JSON 映射文件。
