...
3. 用户可明确选择 `deepseek-v4-flash + max`；只有当前运行面确认 `create_thread` 接受该规范模型名和 `max` 时才使用。若运行时返回规范名为 `deepseek-v4-flash-0731` 等具体 ID，使用返回的规范名，不根据显示名猜测。
4. 若 `gpt-5.6-luna + max` 不可用，且用户未指定其他组合，再查看当前推荐组合，列出最多 3 个可传给 `create_thread` 的替代方案，并请求用户明确确认。
5. 默认组合、用户选择或推荐组合只对当前顶层任务有效；不修改主任务模型、全局默认模型或用户配置。
6. 创建执行任务时显式传入已选定的模型和 effort。模型或 effort 被拒绝时返回原始错误；若拒绝的是默认组合，回到推荐方案确认，不自动换成未经确认的模型。
